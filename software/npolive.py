...
class NPOLive:
	...
	def getUrl(self):
		...
		info, response = http.request("http://ida.omroep.nl/npoplayer/i.js?s=http://www.npo.nl/live/npo-%s" % (self.kanaal))
		if info['status'] == '200':
			...
			url = "http://ida.omroep.nl/aapi/?callback=jQuery18306757661136888681_1440684352682&type=jsonp&stream=http://livestreams.omroep.nl/live/npo/tvlive/ned"+self.kanaal+"/ned"+self.kanaal+".isml/ned"+self.kanaal+".m3u8&token=%s&version=5.1.0&_=%d" % (token, int(time.mktime(datetime.datetime.now().timetuple())) * 1000 - 3504)
			...
			if info['status'] == '200':
				...
				if info['status'] == '200':
					...
					url = url.replace('ned' + self.kanaal + '.m3u8', 'ned' + self.kanaal + '-audio=128000-video=700000.m3u8')
